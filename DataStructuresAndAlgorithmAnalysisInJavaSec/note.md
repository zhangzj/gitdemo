...
1.5之后出现了泛型类PriorityQueue，这个对象可以通过无参数，比较器或者另一个兼容的容器被构造

### 小结

优先队列的用途从操作系统的工作调度到时间模拟

## 7 排序

排序问题可以使整数数组也可以是一般的对象；能够在主存中完成的排序规模比较小叫做内部排序，一定要借助磁盘或者在磁带上完成的排序野蛮重要的，叫做外部排序；

内部排序的考察；存在一些容易的算法，比如插入排序；希尔排序；还有一些性能更高的算法

排序其实不是数据结构，是一种算法，针对排序的很多个不同的算法策略，对于性能会有很大的差别
...
之前的算法都是直接将数据装进内存，但是数据很大的时候，装不进内存，那就需要别的方式；大部分内部排序都用到了内存可以直接寻址的事实；但是访问磁盘会很慢，

基本的外部排序算法使用归并排序中的合并算法

## 8 不相交集类

解决等价问题的一种有效数据结构；使用最小的编程代价来实现

### 8.1 等价关系

等价关系的三个性质，对于元素a，自反性质，对称性质，传递性质；等价关系不是相等，是一种关系的定义，两个城市位于同一个国家，一个城市是自己连通自己的，两个城市之间的连通是双向的，三个城市连通的状态是传递的，那么设个关系的定义就是一个等价关系；

### 8.2 动态等价性问题

### 8.3 基本数据结构

可以使用树来表示每一个集合，这种树的集合可以成为森林；
...
生活中发生的问题可以转化成图论问题；一些算法解决几个常见的图论问题；深度优先搜索

### 9.1 若干定义

一个图就是顶点和变组成的集合，有时候边也叫作弧，边可以认为是顶点的对，一对顶点是不是有序决定这个图有向性；有时候一个边还有第三个属性，叫做权或者值；

一条路径的意思就是经过多个点多个边的一个路径。简单路径是路径上所有的顶点都是互异的；一个圈的意思是满足起点和终点重合的一条路径，无向图中的圈的边是互异的；

如果无向图中的每一个顶点到每一个其他顶点都存在一条路径，那么这个无向图是连通的；连通的有向图称为强连通的，如果一个有向图不是强连通的，去掉方向周可以连通的话那就是若连通的；每一对顶点之间都有边的话，就是完全图

航空系统；边的权值被看作是时间，距离，或者飞行成本；
...
算法给定时候，具体的数据结构无需指定，由dev来选择适当的数据结构

5种通常类型的算法，了解一般的处理方法；

### 10.1 贪婪算法

拿到眼前最好的就可以，贪婪的；哈夫曼编码，近似装箱问题

### 10.2 分治算法

分：递归解决较小的问题；治：然后从子问题解构原问题的解；

### 10.3 动态规划

递归算法写成非递归算法，然后把子问题的答案系统记录在一个表内，这种当大的一个技巧叫做动态规划

矩阵乘法的顺序安排；最优二叉查找树；所有点对最短路径
...
算法设计的五个方法，面对一个问题的时候，考察一下这些方法是不是适用并且值得；算法的适当选择，机上数据结构的审慎使用，通常可以解决问题；

## 11 摊还分析

摊还分析：在摊还分析中，我们求数据结构的一个操作序列中所执行的所有操作的平均时间，来评价操作的代价。这样，我们就可以说明一个操作的平均代价是很低的，即使序列中某个单一操作的代价很高。摊还分析不同于平均情况分析，它不涉及概率，它可以保证最坏情况下每个操作的平均性能。
...
红黑树是具有着色性质的二叉查找树；每一个节点要么是红色，要么是黑色；根节点是黑色的；如果一个接电视红色的，那么他的子节点必须是黑色的；从一个节点到一个null引用的每一条路径必须包含相同数目的黑色节点
